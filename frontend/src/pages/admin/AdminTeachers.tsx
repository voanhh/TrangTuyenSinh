...
import { Plus, Edit, Trash2, X, Loader2, UploadCloud, Search, Filter, Download, MoreVertical } from 'lucide-react';
...
const AdminTeachers: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (isEditing && formData.id) {
                const res = await axios.put(`${API_URL}/api/teachers/${formData.id}`, formData, config);
                if (res.data.status === 200) {
                    setTeachers(prev => prev.map(t => t.id === formData.id ? res.data.data : t));
                }
            } else {
                const res = await axios.post(`${API_URL}/api/teachers`, formData, config);
    ...
    const handleDelete = async (id: number) => {
        if (window.confirm("Bạn có chắc chắn muốn xóa giảng viên này không?")) {
            ...
            const config = { headers: { Authorization: `Bearer ${token}` } };
            try {
                const res = await axios.delete(`${API_URL}/api/teachers/${id}`, config);
                if (res.data.status === 200) {
                    setTeachers(prev => prev.filter(t => t.id !== id));
                    alert("Xóa giảng viên thành công!");
                } else {
                    alert(res.data.message || "Đã xảy ra lỗi khi xóa giảng viên!");
                }
            } catch (error: any) {
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                ...
                <div className="px-6 py-4 border-t border-slate-200 bg-slate-50 flex items-center justify-between">
                    <div className="text-sm text-slate-500">
                        Hiển thị <span className="font-medium text-slate-900">1</span> đến <span className="font-medium text-slate-900">{filteredTeachers.length}</span> trong <span className="font-medium text-slate-900">{teachers.length}</span> giảng viên
                    </div>
                    <div className="flex gap-1">
                        <button className="px-3 py-1 border border-slate-200 rounded-md bg-white text-slate-600 hover:bg-slate-50 disabled:opacity-50 text-sm">Trang trước</button>
                        <button className="px-3 py-1 border border-slate-200 rounded-md bg-white text-slate-600 hover:bg-slate-50 disabled:opacity-50 text-sm">Trang sau</button>
                    </div>
                </div>
            </div>
